...
We include the various reports generated by the test process in the documentation, as you can see at the top of this page in the [Test Reports](#test-reports) section. While the Pylint report is simply included as ReST with some minor filtering to get things to look right, the MyPy and Coverage reports are each displayed with an embedded iframe.
...
Because we include the various reports generated during the test process as part of the documentation — see [here](#test-reports-pages) for more info — we have to find a way to get those reports into the Read the Docs environment. In the `docs/source/conf.py` config file for Sphinx, we execute a `main()` function when running there, detected with the `READTHEDOCS` environment variable. That `main()` function polls Github's REST API for artifacts that match the current git SHA, downloading the reports artifact once it's found.
